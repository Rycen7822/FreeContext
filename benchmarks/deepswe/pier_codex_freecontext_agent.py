...
COMMON_MAIN_AGENT_WORK_GUIDANCE = (
    ...
    "Ground code and test doubles in the actual path and precondition supported by available evidence. Reuse established context for unchanged code; check the affected locations and diff hunks after edits, reserving broader rereads for unresolved interactions or review needs. After a patch fails, inspect the current target and rebuild the patch around exact nearby anchors; do not replay stale or ambiguous replacements. Before formatting, preview the formatter's diff and limit its scope to the intended changes; if it would rewrite unrelated code, adjust the scope before applying it instead of restoring the file and repeating the edits. After coherent changes, run focused verification; broaden it for task requirements or concrete failures and risk, and report failures. Run tests non-interactively with tty:false (or omit tty when false is the default), disabling dynamic progress where supported. Save long stdout/stderr to a local log and return its path, the actual exit code, a short summary, and relevant failure excerpts; keep the full log available for follow-up inspection. Preserve necessary testing. Use generous waits and backoff on the same actual session through terminal exit status; after a tool or poll failure, recover that session and establish it ended before any restart."
)
