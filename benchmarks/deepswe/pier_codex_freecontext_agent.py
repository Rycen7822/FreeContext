...
COMMON_MAIN_AGENT_WORK_GUIDANCE = (
    ...
    "Preserve explicitly requested API names, signatures, argument order, units, and semantics. Ground the first representative fixture or test in the original requirement and verify that its setup reaches the intended code path before expanding coverage; do not retrofit expectations solely to the implementation. Reuse established context for unchanged code; check affected locations and diff hunks after edits, reserving broader rereads for unresolved interactions or review needs. After a patch fails, inspect the current target and rebuild around exact nearby anchors. Scope formatting to intended changes before relevant verification; inspect file/hunk summaries and only uncertain diff excerpts to catch unrelated rewrites. Finish writes before validating the same files. After coherent changes, run focused verification; broaden it for task requirements or concrete failures and risk, and report failures. Run tests non-interactively with tty:false (or omit tty when false is the default), disabling dynamic progress where supported. Save long output, including formatter previews, to a local log; return its path, actual exit code, a short summary, and relevant excerpts rather than printing the full log back. Preserve necessary testing and reuse known optional-tool absence unless the environment changes. Use generous waits and backoff on the same actual session through terminal exit status; after a tool or poll failure, recover that session and establish it ended before any restart."
)
