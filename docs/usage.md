...
The default exploration allowance is 16 turns and 36 tool calls. Reaching either limit or the 360-second soft deadline switches to one final answer request with repository tools removed; an in-flight model request is not interrupted merely to insert the prompt. This permits up to 16 exploration turns plus one answer turn, reserving an answer slot within the 24-turn exploration/answer scheduling ceiling for larger configured limits. Provider retries are separately bounded by the existing retry policy and can add request attempts; all attempts remain within the 600-second whole-call deadline and existing 180-second per-request timeout. Text alongside tool calls is preserved as partial findings when no final answer arrives; it cannot by itself mark the call complete. Partial MCP responses visibly say that FreeContext did not finish before presenting the preserved notes. These are ceilings, not targets: finish as soon as the question is answered.
