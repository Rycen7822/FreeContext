...
- 关键 benchmark 数据：None yet. Candidate v4 has no correctness, Main-Agent token, FC token, total-system, wall-time, or reentry result. It must not be compared as if it had run.
- 结论/下一步：The source is ready for a separately authorized fresh paired benchmark preparation. The next benchmark should test no-FC versus Candidate v4 on the fixed five-task denominator with the same repetitions and concurrency used by the retained baseline; correctness loss is the first stop gate, followed by Main-Agent tokens, steps, and whether FC replaces auxiliary native exploration. Do not raise call frequency if the candidate misses those gates.
- 已知边界：`high` may improve tool reliability but may also increase FC-side tokens or latency. The conservative caller rule is intended to reduce unnecessary calls, but only a fresh paired run can determine the net effect. No task-specific rule or five-task exception was added.
