...
After an edit or failed check, use a diagnostic checkpoint: at most one direct read of the exact failure location is allowed. Before a second non-adjacent file read or cross-module search for that diagnosis, stop and classify the remaining gap. Call FreeContext only when that gap is a distinct new static cross-module relationship not covered by the prior handoff; otherwise stay native. Single-file work, exact failures, routine checks, and the same handoff gap remain native. Never force a second call.
...
For `not_found`, make the one exact probe required by `nextAction`, reuse the prior call's exact `taskText`, `workUnit`, `knownRefs`, and `evidenceQuestions`, and add `{recovery:{priorSessionId:<exact nextAction.recovery value>,probePath:<probed path>}}`. Do not resend constant tags or duplicate the work unit inside recovery. Recovery is once-only and invalid after partial, ready, failure, broad exploration, or bypass. With a handoff use typed reentry; fix `INVALID_REQUEST`; never use broad fallback.
