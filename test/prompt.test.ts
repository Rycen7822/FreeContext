...
test("default explorer reserves the fourth turn for terminal submission", async () => {
  ...
  assert.match(prompt, /targeted-search wave seeks enough distinct line or symbol candidates to satisfy each required question's `minimumSpans`/u);
  assert.match(prompt, /globbed paths are not content candidates/u);
  assert.match(prompt, /Turn 2: read one role-matched candidate per required question, then continue round-robin/u);
  ...
  assert.match(prompt, /Stop when each required question has its requested number of distinct role-matched decisive spans/u);
...
test("user prompt preserves the exact task and renders typed evidence inputs", () => {
  const request: FreeContextRequest = {
    ...
    evidenceQuestions: [
      { id: "impl", role: "implementation", question: "Where is x implemented?", required: true, minimumSpans: 2 },
  ...
  assert.match(prompt, /\[implementation\]\[impl\]\[required\]\[minimum-spans=2\] Where is x implemented\?/u);
