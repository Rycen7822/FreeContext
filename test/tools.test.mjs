...
import { createWorkspace, MAX_DIRECT_FILE_BYTES } from "../src/tools/workspace.mjs";
import { createRepositoryTools, detectToolExecutables } from "../src/tools/index.mjs";
import { FakeType, baseConfig } from "./helpers.mjs";
...
test("read, rg, and glob expose bounded repository evidence", async () => {
  ...
  try {
    ...
    assert.match(read.content[0].text, /1: export function target/u);
    assert.match(read.content[0].text, /3: \}/u);
    ...
    assert.match(envExample.content[0].text, /SECRET=/u);
    ...
    assert.match(rg.content[0].text, /src\/app\.js:1:/u);
    assert.match(rg.content[0].text, /src\/app\.js:5:/u);

    const glob = await findTool(toolset.tools, "glob").execute("glob-1", {
      pattern: ["**/*"],
      path: ".",
    });
    assert.match(glob.content[0].text, /src\/app\.js/u);
    assert.doesNotMatch(glob.content[0].text, /\.env\.example/u);
    assert.doesNotMatch(glob.content[0].text, /(^|\n)\.env($|\n)/u);
  } finally {
...
test("model globs cannot re-include protected files", async () => {
  ...
  try {
    ...
    assert.match(rg.content[0].text, /<no matches>/u);
    assert.doesNotMatch(rg.content[0].text, /SECRET=hidden/u);
    ...
    assert.match(explicitSafeExample.content[0].text, /\[rg path=\.env\.example\]\n1:1:/u);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
