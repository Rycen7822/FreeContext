...
test("CLI text and JSON output remain ordinary text and write the benchmark session", async () => {
  ...
  await writeFile(path.join(root, "capture.txt"), "  src/file.ts:7 evidence\n");
  try {
    for (const format of ["text", "json"] as const) {
      const stdout: string[] = [];
      const stderr: string[] = [];
      const stdin = Readable.from(["  src/file.ts:7 evidence\n"]) as CliIo["stdin"];
      const code = await executeCli([
        "--cwd", workspace,
        "--format", format,
        "--benchmark-session-file", sessionFile.replace("call.json", `${format}.json`),
        "--intent", "Extract the implementation evidence",
        ...(format === "json" ? ["--output-file", path.join(root, "capture.txt")] : []),
      ], {
        stdin,
        stdout: sink(stdout),
        stderr: sink(stderr),
      }, {
        runExplorer: async ({ invocation, request }) => {
          assert.deepEqual(request, { intent: "Extract the implementation evidence", output: "  src/file.ts:7 evidence\n" });
          return ({
          status: "complete",
          text: "plain answer",
          errorCode: null,
      ...
      assert.match(output.text, /Session: [^\n]+\n?$/u);
      ...
      assert.match(document.result?.text ?? "", /^plain answer\n\nCaptured output:/u);
      assert.equal(await readFile(`${sessionFile.replace("call.json", `${format}.json`)}.output.txt`, "utf8"), "  src/file.ts:7 evidence\n");
    }
  } finally {
