...
export const TOOL_DESCRIPTION = [
  ...
  "At any phase, delegate one unresolved code-fact or relationship question when answering it needs substantial new reading: a definition, caller, value flow, or specific behavior. Keep architecture decisions and the overall diagnosis or fix yourself; do not ask FC to design the feature or certify the whole implementation.",
  "Stay native when the answer is already in context or needs only a small direct check. Known paths or an exact error location do not by themselves make an investigation small. Do not call merely because a task starts.",
  ...
  "Native work can reveal an unresolved relationship; if resolving it needs substantial new reading, delegate it with checked facts and any relevant change or failure. A failed test or new module is not required. Do not request a full re-audit, repeat a resolved question, or call for an obvious local fix.",
  "The worker returns ordinary assistant text after tracing the implementation or relationship that decides the requested behavior, with a conclusion, verified path:line-line, function or symbol, and a short decisive excerpt when useful. Wording is not schema-validated or quota-fitted.",
...
export const SERVER_INSTRUCTIONS = [
  "FreeContext accepts one question and optional hints; delegate one unresolved code-fact or relationship investigation whose answer needs substantial new reading, not a whole-feature design or correctness audit.",
  ...
  "Keep exact edit-context reads, design decisions, edits, and tests in the main agent. During native work, a specific unresolved relationship can justify another bounded investigation; small direct fixes remain native.",
].join(" ");
