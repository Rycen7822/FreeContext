...
export function sanitizedToolEnv(extra = {}) {
  return {
    PATH: DEFAULT_PATH,
...
export async function findExecutable(names, envPath = DEFAULT_PATH) {
...
export async function runCommand({
  ...
  return await new Promise((resolve, reject) => {
    ...
    const append = (target, chunk, currentBytes, setter) => {
    ...
    child.stdout.on("data", (chunk) => append(stdout, chunk, stdoutBytes, (value) => { stdoutBytes = value; }));
    child.stderr.on("data", (chunk) => append(stderr, chunk, stderrBytes, (value) => { stderrBytes = value; }));
